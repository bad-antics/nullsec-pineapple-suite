...
# Network
WLAN0_STATUS=$(iw dev wlan0 info 2>/dev/null | grep -c "ssid" && echo "UP" || echo "DOWN")
WLAN1_STATUS=$([ -d /sys/class/net/wlan1 ] && echo "UP" || echo "DOWN")
MON_STATUS=$([ -d /sys/class/net/wlan1mon ] && echo "ACTIVE" || echo "OFF")
IP_ADDR=$(ip -4 addr show wlan0 2>/dev/null | grep -oP 'inet \K[\d.]+' | head -1 || echo "N/A")
